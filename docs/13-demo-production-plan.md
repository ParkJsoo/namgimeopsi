...
작성: 2026-09-08 / 갱신: 2026-09-09. **전용 시뮬레이터 Release·10초 시험 녹화·재생·사진 선택 검증 완료.** 전체 리허설도 완료했다. 본촬영 전 날짜 저장·표시 불일치 수정이 필요하며 최종 영상은 아직 없다.
...
현재 진행 지점은 **전체 리허설·재실행 검증 완료**다. [리허설 결과](14-demo-rehearsal.md)의 날짜 저장·표시 문제를 별도 앱 수정으로 해결한 뒤 본촬영 준비를 한다. 아래 시험 단계의 재고 5개·검수 진입 상태는 과거 기준이며 현재는 저장 lot 11개·활성 10개·원장 7건이다.
...
- 날짜 관련 구현 불일치가 있으므로 본촬영 전에 앱 수정·검증이 필요하다. 준비·리허설 문서와 해당 앱 수정은 별도 PR 범위로 유지한다.
- 본촬영은 리허설 종료 재고를 초기화하지 않는 별도 세션에서 준비한다. 새 촬영 세션은 아직 생성하지 않았다.
