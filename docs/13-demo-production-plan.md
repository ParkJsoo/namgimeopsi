...
작성: 2026-09-08 / 갱신: 2026-09-14. 환경·시험 녹화·전체 리허설·날짜 후속 수정 완료. 120초 로컬 편집본까지 완료했으며 Demo에는 본촬영 결과 재고가 남아 있다.
...
1. 최신 인계와 [제작 결과](17-demo-production-result.md), 전문 리뷰 결과를 확인한다.
2. 현재 Demo의 본촬영 결과 재고와 영상 원본을 보존한다. 환경 생성·재고 초기화·촬영을 반복하지 않는다.
3. 리뷰에서 발견한 코드·영상 개선점을 반영할 경우 해당 변경의 회귀와 영상 영향을 확인한다.
4. 완성본 사용자 검토 후 공개처·업로드와 README 공개 링크를 정한다. 외부 공개는 아직 하지 않았다.

## 장면 구성

세부 문구는 [120초 대본](08-portfolio-demo.md)을 사용한다. 다음은 초기 목표 구간이며 최종 구간은 제작 결과의 시간표를 따른다.
...
1. **데모 준비·리허설 PR:** 오늘의 인계/계획 문서, 다음 세션의 촬영 재현 절차·확정 대본·리허설 결과·필요한 최소 촬영 도구. 준비 커밋은 `docs/demo-rehearsal-handoff`에서 시작했고 현재 `fix/inventory-calendar-dates`가 이를 포함한다. 원격 PR은 아직 없다.
...
- 2026-09-08 읽기 전용 확인: `/usr/bin/xcrun`, `/opt/homebrew/bin/ffmpeg`, `/opt/homebrew/bin/ffprobe` 존재. `xcrun simctl help io`에서 `recordVideo`와 H.264 출력 지원을 확인했다. 당시에는 실제 녹화·편집 전이었고, 현재는 제작 결과에 검증을 기록했다.
- iOS QA 근거: `.expo/ios-gesture-qa/`, `.expo/ios-review-qa/`. 기존 테스트 시뮬레이터 lot 6개·원장 2개는 9월 14일 사용자 승인으로 삭제했고, QA 근거 파일은 보존했다.
