...
- **현재 화면:** 6개 샘플 품목의 검수 화면, 아직 수정·제외·입고하지 않았다. 새 설치 초기 lot 5개·원장 0개·outbox 0개를 확인하고 `.expo/demo-production/inventory-before-rehearsal.json`에 기준 재고를 보관했다. 홈 순위는 애호박 두부덮밥 → 치킨마요 덮밥 → 만두 계란국이다.
- **다음 작업:** 같은 데모 세션에서 참기름 `조금 남음` 수정·돼지고기 제외 → 5개 입고 → 남은 카레 등록 → 조리 부분/전량 차감 리허설. lot ID별 전후 수량과 소요 시간을 기록한다. 초기 날짜는 고정 fixture이므로 상대 날짜 안내를 실제 촬영일 기준 안전 판단으로 설명하지 않는다. 세 흐름 전체 리허설·본촬영·최종 영상·공개는 아직 미완료다.
