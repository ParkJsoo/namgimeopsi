...
- 구현 상태: Expo TypeScript 앱에서 영수증 검수형 입고, 남은 음식 등록, 메뉴 추천·전량 소비를 구현했다. 재고는 Supabase 익명 사용자별 Postgres에 동기화하고, 기존 AsyncStorage 상태는 최초 원격 시드와 오프라인 캐시로 유지한다. private 원본 이미지·scan job·서버 검증 마이그레이션은 실제 프로젝트에 적용했고 `analyze-receipt` 최신 로컬 원문도 Dashboard에서 배포했다. 그러나 `scan_jobs`의 UPDATE 권한이 `service_role`에도 없어 운영 함수가 `scan_state_update_failed`로 멈춘다. 권한 보정 마이그레이션을 추가했지만 Dashboard 적용과 최종 익명 E2E가 남아 있다. OCR provider는 아직 연결하지 않았다.
...
- `20260901010000_receipt_scan_jobs.sql`은 SQL Editor로 실제 프로젝트에 적용했다. 이번 세션에서 `npx --yes deno check supabase/functions/analyze-receipt/index.ts`를 통과시키고, Dashboard `analyze-receipt > Code`의 편집기를 로컬 원문(5,213 bytes)과 일치하는지 확인한 뒤 `Deploy updates`를 완료했다. 새 익명 사용자로 private PNG 업로드와 `scan_jobs` 생성은 재확인했으나 함수는 `scan_state_update_failed`(500)를 반환했다. Dashboard SQL의 읽기 전용 확인에서 `has_table_privilege('service_role', 'public.scan_jobs', 'UPDATE')`가 `false`였고, 원인은 privileged 함수 클라이언트에도 필요한 테이블 권한 부재로 확정했다. `20260902000000_allow_scan_function_state_updates.sql`에 service_role 전용 UPDATE 권한 보정을 추가했다. Dashboard에서 이 migration SQL을 실행한 뒤, 새 익명 사용자로 private upload → fixture `ready` job → `commit_receipt_scan_intake` → 무인증 `/object/public/` 원본 차단을 다시 검증해야 한다. OCR provider나 키는 연결하지 않았고 fixture 결과라는 UI 고지는 유지한다. Expo lint 복구를 위해 `eslint`·`eslint-config-expo`를 dev dependency로 명시했으며, Expo 57/TypeScript 6 resolver 호환 문제의 import 규칙은 `tsc --noEmit` 검증으로 대체했다.
...
2. Dashboard SQL Editor에서 `supabase/migrations/20260902000000_allow_scan_function_state_updates.sql`의 `grant update on table public.scan_jobs to service_role;`를 적용한다. 적용 뒤 새 익명 사용자로 private 업로드 → fixture `ready` scan job → `commit_receipt_scan_intake` 실제 호출 → 인증 없는 원본 조회 차단을 검증한다. `analyze-receipt` 최신 원문 배포와 Deno 검사는 이미 완료했다. OCR provider와 키를 연결하기 전까지 fixture 결과라는 UI 고지를 유지한다.
