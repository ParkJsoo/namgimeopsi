...
- Galaxy 남은 음식 저장: 사용자가 `QA 카레`·`2인분`·`냉장`으로 저장한 뒤 냉장고 목록에 표시됨을 확인했다. 사용자 본체 보고 기준으로 저장·목록 표시는 통과이며 테스트 항목은 유지한다. 바로 다음은 앱 종료·재실행 후 해당 항목 유지 확인이다. 원격 동기화 완료는 별도 검증하지 않았다.
