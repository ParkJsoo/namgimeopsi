...
- 영상 P3 편집과 공개 크기 검수를 진행 중이다. 본촬영 원본과 기존 120초 영상은 보존하며 별도 `.expo/demo-production/p3-edit/`에 편집한다. 기존 파일 SHA-256 `29a203a37b8fb85f3b9f24605fab50b96457d5eef78745c3a1a111f054f9eca0`를 확인했다.
- 실기기·시뮬레이터·다른 워크스페이스·Figma·서버 데이터에 접근하지 않았다. Demo 본촬영 데이터는 조작하지 않는다. push·PR·외부 업로드는 하지 않는다. 별도 리뷰 문서는 만들지 않는다.
- 다음: 별도 편집본 검수 완료 → 사용자 최종 검토·공개처 결정. 실제 공개 위치의 임베드 크기 확인·업로드·공개 링크 연결은 그 뒤다.
