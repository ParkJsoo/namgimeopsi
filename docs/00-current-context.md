...
## 최신 작업 — 2026-09-15 P2 수정 진행

- `docs/final-check-handoff` / `982ab26`의 clean HEAD에서 `fix/final-check-p2`를 만들었다. 기존 인계 커밋은 모두 유지한다.
- ① 공백 수량을 화면·hook에서 차단하고, 기존 잘못된 upsert는 같은 lot의 후속 정상 수정/제외로 대체된 경우에만 정리한다. 다른 lot·영수증/조리 트랜잭션·원장은 보존한다. 미확정 수량을 추정하지 않는다.
- 수정 전 재현과 새 회귀 실패 4건을 확인했고 수정 후 inventory 13 + 날짜 9개를 통과했다. 아래 종료 인계의 P2 미수정 표시는 당시 상태이며, 나머지 ②~⑤는 이어서 수정한다.
- ② 영수증 저장 중 이름·수량·날짜·보관 위치·제외와 중복 제출/닫기를 차단하고 완료 수량은 제출 스냅샷에서 계산한다. 실패/예외 뒤에는 편집을 다시 허용한다. 실제 컴포넌트의 성공·실패·예외 회귀와 기존 업로드/분석 닫기 검사를 통과했다. ③~⑤는 다음 순서다.
- ③ v2·legacy·outbox 캐시 읽기 실패를 잡아 시작 화면에 재시도를 제공한다. 읽기 성공 전 쓰기/원격 요청을 하지 않으며 중복 재시도는 합친다. 오프라인 초기 쓰기 실패도 잡아 읽은 상태를 보존한다. inventory 17 + 날짜 9, 타입·lint 통과. ④~⑤는 다음 순서다.
- ④ 저장/동기화 안내와 재시도를 홈·냉장고 탭 공통 영역으로 옮겼다. 로컬 저장 실패도 포함하는 문구로 바꾸고 오류는 alert로 노출한다. 실제 화면 컴포넌트의 초기 읽기 실패·추가/수정 수량·양쪽 탭 상태 6개 회귀, 타입·lint 통과. ⑤ CORS를 이어서 수정한다.
- 실기기·시뮬레이터·Figma·다른 워크스페이스·원격 데이터에 접근하지 않았다. 두 로컬 영상의 SHA-256은 기존 기록과 같다.
