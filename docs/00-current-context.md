...
## 최신 인계 — 2026-09-14 동명 lot 선택 P2 수정 완료
...
- **미해결 P2:** 동명 lot가 여러 개면 추천은 가장 긴급한 lot의 상태를 사용하지만 차감 시트는 배열의 첫 lot를 선택한다. 새 두부 9/20·기존 두부 9/13 예에서 기존 두부 때문에 날짜 경과 안내가 뜨지만 새 두부가 차감되는 것을 재현했다. 기존 결함이며 현재 촬영의 단일 lot 데이터에는 영향이 없다. 다음 코드 작업의 우선순위다.
...
- 다음은 동명 lot 선택 일관성·구별 표시 보완과 회귀 검증, 공개 크기에서 영상 확인, 사용자 최종 검토·공개처 결정이다. 현재 영상 파일과 촬영 재고는 아래와 같다.
