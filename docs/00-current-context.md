...
4. 사용자 요청으로 전용 iOS 26.5 시뮬레이터 Release에서 세 시트의 소프트웨어 한글 조합·마지막 필드/버튼 표시, 남은 음식 저장·조리 부분/전량 차감·재실행 유지를 확인했다. 업로드/분석 중 닫기·재진입은 임시 15초 성공 응답 전달 지연 조건에서 확인했다. 최종 시뮬레이터 앱은 지연 없는 원래 Release로 복원했다. 실제 iPhone 본체의 해당 흐름과 손가락 스와이프는 여전히 별도 QA 대상이며, 상세 한계는 `docs/09-device-qa.md`를 따른다.
