...
## 최신 인계 — 2026-09-14 전문 재리뷰: 재진입 수량 초안 P2 발견

- 전문 에이전트 2명이 `d9bd8ce`를 코드·화면 관점에서 재검토했다. 기존 동명 lot 선택 불일치는 해결 판정이다. [전체 결과](20-lot-selection-review.md).
- **미해결 P2:** 두부 1모 → 반 모 소비 후 같은 메뉴를 다시 열면 `남은 양`에 예전 1모가 남아 재고를 증가시키는 소비를 저장할 수 있다. 실제 시트·원장 연결 모의 호스트에서 닫힘·갱신된 재고·재오픈으로 재현했다. 다음 우선순위는 세션 초안 무효화와 이 경계의 회귀 테스트다.
- 기존 recipes 6 / inventory 8 + dates 9 / domain 14 / receipts / TypeScript·lint는 통과하지만 새 경계는 포함하지 않는다. 화면 캡처 3장은 기존 선택 결과와 일치한다. 등록 시각의 접근성 라벨·소비 방식 선택 상태는 P3 후보이며 실제 VoiceOver는 미검증이다.
- 이번에는 리뷰 기록만 갱신했다. 앱 소스·본촬영 재고·영상은 유지하고 실기기를 사용하지 않았다. 영상의 구버전 고지는 정확하다. push·PR·외부 공개는 하지 않았다.
...
## 이전 리뷰 — 2026-09-14 전문 에이전트 2명 검토 완료

- 코드·회귀 테스트 담당과 영상·편집 담당이 독립 검토했다. [전체 결과](18-specialist-review.md).
- 앞선 두 날짜 P2 수정은 정상이며 inventory 8 + 날짜/추천 9 테스트 통과. 추가 P1 또는 새 날짜 회귀는 발견하지 못했다.
- **당시 발견 P2 — 후속 수정 완료([검증](19-lot-selection-qa.md)):** 동명 lot가 여러 개면 추천은 가장 긴급한 lot의 상태를 사용하지만 차감 시트는 배열의 첫 lot를 선택한다. 새 두부 9/20·기존 두부 9/13 예에서 기존 두부 때문에 날짜 경과 안내가 뜨지만 새 두부가 차감되는 것을 재현했다. 기존 결함이며 현재 촬영의 단일 lot 데이터에는 영향이 없다. 이후 `d9bd8ce`에서 수정했다.
- 영상은 핵심·경계 프레임 23장과 전체 디코딩 독립 검토에서 P2 이상 발견 없음. 70–72초 컷 전환과 작은 임베드에서의 글씨 크기는 P3 개선 후보다. 영상 해시·길이·데이터 근거는 기록과 일치한다.
- README·대본·제작 계획에 남은 “영상 미제작” 및 옛 재고 보존/촬영 순서를 정리했다. 이번 리뷰에서는 앱 코드·영상·서버 데이터를 변경하지 않았다.
- 당시 다음 작업이었던 동명 lot 선택·구별 표시 보완과 회귀 검증은 완료했다. 공개 크기에서 영상 확인, 사용자 최종 검토·공개처 결정이 남는다. 현재 영상 파일과 촬영 재고는 아래와 같다.
...
## 이전 리뷰 — 2026-09-14 후속 검토

- `d7e62dc`를 검토하고 domain·inventory·receipts·recipes·TypeScript·lint를 다시 실행해 모두 통과했다. 다만 별도 재현에서 아래 미해결 사항을 확인했으므로 본촬영 전에 보완한다. 상세: [리뷰와 남은 작업](16-review-and-next-steps.md).
- **P2:** 예제 lot의 권장일을 지운 뒤 오프라인 재실행하면 `mergeMissingRecommendationDates`가 예전 시드 날짜를 다시 넣는다. 기존 hook 테스트는 hydration을 생략해 이 경계를 검증하지 않는다.
- **P2:** 추천은 `overdue`를 임박 재료와 함께 가점 처리하고 “먼저 쓰기 좋아요”라고 설명한다. 날짜가 지났다는 정보와 상태 확인 조건이 추천 문구에서 사라진다. 홈·목록 문구만 보정한 상태다.
- **촬영 대본 보정:** 현재 Demo는 빈 재고지만 `docs/08` 대본에는 “기존 애호박 반 개는 그대로”가 남아 있다. `docs/13`의 다음 순서에도 이미 끝난 환경 생성·시험 녹화와 옛 데이터 보존 지시가 남았다. 현재 데이터 기준으로 대본·실행 순서를 갱신해야 한다.
- 이번 턴은 검토·재현·문서 기록만 수행했다. 앱 코드와 서버 데이터는 변경하지 않았다. 다음 순서는 날짜 삭제 hydration 회귀 수정 → 지난 날짜 추천 설명 보완 → 빈 재고 기준 대본 재확인 → 본촬영·편집·최종 검수다.
