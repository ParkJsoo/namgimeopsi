...
4. 전용 iOS 26.5 시뮬레이터의 세 시트 한글 조합·키보드 회피에 이어, 실제 iPhone 미러링에서 영수증 끝까지 스크롤·사진 선택 취소/재진입·검수 취소·남은 음식 저장·조리 취소/부분·전량 차감·재실행 유지를 확인했다. 진행 중 닫기·재진입은 시뮬레이터 15초/실제 기기 20초 성공 응답 전달 지연 조건에서 확인했다. 두 앱 모두 지연 없는 원래 Release로 복원했다. 남은 것은 **실제 본체 소프트웨어 한글 키보드·키보드를 연 채 직접 손가락 스와이프**이며, 미러링/시뮬레이터 결과로 대체하지 않는다. 상세는 `docs/09-device-qa.md`를 따른다.
5. iOS preview는 로컬 Apple Development 서명된 Release의 실제 설치·cold launch와 미러링 저장·소비까지 확인했다. EAS·TestFlight·스토어 배포가 아니며, 본체 화면 키보드 QA는 별도로 남아 있다.
...
- 본체 키보드 결과 보고 대신 사용자가 시뮬레이터 검증을 요청해 진행했다. 전용 `Namgimeopsi QA iPhone 13 mini`만 사용했다. `QA SIM 카레 2인분`, 두부 `반 모`, 애호박 `consume-all` 원장을 보존한다. 이번 Computer Use의 스와이프·휠 조작은 일관되게 동작하지 않아, 접근성 포커스에 따른 자동 스크롤·키보드 회피 결과와 구분한다.
...
- **시뮬레이터 후속:** `bb3258e` 이후 문서 외 영구 소스 변경 없이 전용 iOS 26.5 Release QA를 수행했다. 세 시트 한글 조합, 남은 음식 저장·소비 원장 2건·재실행 유지, 응답 지연 조건의 두 진행 단계 닫기·재진입을 확인했다. 원래 소스·Release를 복원하고 번들 해시 일치·Metro 없는 실행·최종 목록을 재확인했다. 본체 및 스와이프 검증으로 확대 해석하지 않는다.
